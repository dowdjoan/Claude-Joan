...
const fillShiftX = (keyframes: CameraKeyframe[]): Required<CameraKeyframe>[] => {
  let lastShift = 0;
...
export const makeCameraTransformGetter = (keyframes: CameraKeyframe[]) => {
  const filled = fillShiftX(keyframes);
  return (timeSec: number): { scale: number; shiftX: number } => {
    let curr = filled[0];
    let prev = filled[0];
